...
Since A* expands in a fairly conservative fashion from the start state onward, the start-to-frontier estimates are computed quite accurately by construction of a valid path.
Thus, the principal issue is the derivation of frontier-to-goal estimates.
Given exact distances, it would be a simple matter to trace a valid path from the start, successively taking optimal successors until a goal is reached.
...
The resulting weighted A* search is complete and has a configurable suboptimality guarantee.
